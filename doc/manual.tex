...
\usepackage{tikz,xcolor,booktabs}
...
\begin{figure}
  ...
  \begin{tikzpicture}[%
    ...
    \node at (2*\dist,0) (g) {g};
    \node at (3*\dist,0) (t) {t};
    \node at (-1*\dist,2*\dist) (s) {s};
    \node at (0*\dist,2*\dist) (w) {w};
    \node at (1*\dist,2*\dist) (r) {r};
    \node at (2*\dist,2*\dist) (y) {y};
    \node at (3*\dist,2*\dist) (k) {k};
    \node at (4*\dist,2*\dist) (m) {m};
    \draw (a) -- (w);
    \draw (a) -- (m);
    \draw (a) -- (r);
    \draw (c) -- (s);
    ...
    \draw (t) -- (k);
    \draw (t) -- (y);
    \node at (0*\dist,4*\dist) (b) {b};
    \node at (1*\dist,4*\dist) (d) {d};
    \node at (2*\dist,4*\dist) (h) {h};
    \node at (3*\dist,4*\dist) (v) {v};
    \node at (1.5*\dist,6*\dist) (n) {n};
    \draw (s) -- (b);
    \draw (k) -- (b);
    \draw (y) -- (b);
    \draw (r) -- (d);
  ...
  \begin{tabular}[t]{ccc}
    ...
    \bottomrule
  \end{tabular}
  \caption[IUPAC nucleic acid code]{%
    The IUPAC code for nucleic acids.
  }
\end{figure}
...
\subsubsection{Multiple, independent seeds}
...
\subsubsection{HMM of multiple motifs}
