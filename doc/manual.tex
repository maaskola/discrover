...
By default, \verb|discrover| will generate sequence logos in \verb|PNG| and \verb|PDF| format.
While \verb|plasma| does not by default generate sequence logos, the command line switches \verb|--png| and \verb|--pdf| respectively generate sequence logos in \verb|PNG| and \verb|PDF| format.

You can also use the same routines via the separate program \verb|discrover-logo| to generate sequence logos from the \verb|.hmm| files generated by \verb|discrover|, from IUPAC regular expressions, and from motif matrices in files:\\
\begin{verbbox}
...
Note that, as \verb|tikzlogo| requires a relatively large \LaTeX{} environment to be installed, this script has now been deprecated.
