...
In short, hybrid learning optimizes the emission parameters so as to increase the chosen objective function, while the transition parameters are optimized for the likelihood.

Aside from the hybrid learning mode, \discrover{} offers two other modes, determined by the \verb|--bglearn| command line switch.
With \verb|--bglearn fixed| the transition parameters are left in their initial state and not optimized, while with \verb|--bglearn gradient| the transition parameters are optimized for the same chosen objective function.
...
For this reason, MCMC learning in \discrover{} optimizes all parameters, including the transition parameters, for the chosen objective function.

There are several options available to determine the way \discrover{} performs MCMC
For these, please refer to the \emph{Gibbs learning options} section of the extended command line help, available via \verb|discrover --help --verbose|.
