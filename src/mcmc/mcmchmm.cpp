    ...
    class Evaluator<HMM<alphabet_size>> {
      ...
      public:
        Evaluator(const std::vector<DataSet> &signal,
            const std::vector<DataSet> &control,
            ObjectiveFunction fnc) :
          signal_data(signal),
        ...
        {
          double score = -hmm.compute_score(signal_data, control_data, objFunc, signal_data_size, control_data_size);
