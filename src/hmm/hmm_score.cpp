...
ostream &operator<<(ostream &os, const HMM::pair_posterior_t &p) {
  os << "logL = " << p.log_likelihood << " A = " << p.posterior_first
     << " B = " << p.posterior_second << " AB = " << p.posterior_both
     << " not(AB) = " << p.posterior_none << endl;
  os << p.posterior_both << "\t" << p.posterior_first - p.posterior_both << endl
     << p.posterior_second - p.posterior_both << "\t" << p.posterior_none
     << endl;
  return os;
}

confusion_matrix HMM::reduce(const vector_t &v, bitmask_t present,
  ...
  for (size_t sample_idx = 0; sample_idx < v.size(); sample_idx++)
    if (is_present(contrast.sets[sample_idx], present)) {
      ...
      m.false_negatives += (word_stats ? contrast.sets[sample_idx].seq_size
                                       : contrast.sets[sample_idx].set_size)
                           - v[sample_idx];
    } else {
      m.false_positives += v[sample_idx];
      m.true_negatives += (word_stats ? contrast.sets[sample_idx].seq_size
                                      : contrast.sets[sample_idx].set_size)
                          - v[sample_idx];
    }
  return m;
...
double HMM::mutual_information(const Data::Contrast &contrast,
  ...
  if (verbosity >= Verbosity::debug)
    cout << "HMM::mutual_information(Data::Contrast)" << endl
         << "present = " << present << endl << "counts  = " << m << endl;
...
pair<double, double> HMM::conditional_and_motif_pair_mutual_information(
    const Data::Contrast &contrast, bitmask_t present,
    bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
    cout << "HMM::conditional_and_motif_pair_mutual_information(Data::Contrast)"
         << endl;
  auto pair_posteriors
      = pair_posterior_atleast_one(contrast, present, previous);
  double conditional_mi = calc_conditional_mutual_information(
      contrast, pair_posteriors, pseudo_count, verbosity,
      conditionalMI::motif_and_condition_given_previous_motifs);
  double pair_mi;
  if (use_conditional_pair_mi)
    pair_mi = calc_conditional_mutual_information(
        contrast, pair_posteriors, pseudo_count, verbosity,
        conditionalMI::motif_and_previous_motifs_given_condition);
  else
    pair_mi = pair_mutual_information(contrast, pair_posteriors, pseudo_count,
                                      verbosity);
  if (verbosity >= Verbosity::verbose)
    cout << "HMM::conditional_and_motif_pair_mutual_information(Data::Contrast)"
         << endl << "present  = " << present << endl
         << "previous = " << previous << endl << "condMI = " << conditional_mi
         << " pairMI = " << pair_mi << " ratio = " << (conditional_mi / pair_mi)
         << endl;
  return make_pair(conditional_mi, pair_mi);
}

double HMM::conditional_mutual_information(const Data::Contrast &contrast,
                                           bitmask_t present,
                                           bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
  ...
  if (verbosity >= Verbosity::verbose)
    cout << "HMM::conditional_mutual_information(Data::Contrast)" << endl
         << "present  = " << present << endl << "previous = " << previous
         << endl << "condMI = " << conditional_mi << endl;
  return conditional_mi;
}

double HMM::motif_pair_mutual_information(const Data::Contrast &contrast,
                                          bitmask_t present,
                                          bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
  ...
  if (verbosity >= Verbosity::verbose)
    cout << "HMM::conditional_mutual_information(Data::Contrast)" << endl
         << "present  = " << present << endl << "previous = " << previous
         << endl << "pairMI = " << pair_mi << endl;
  return pair_mi;
}

double HMM::rank_information(const Data::Contrast &contrast,
                             bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
...
double HMM::rank_information(const Data::Set &dataset,
                             bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
...
double HMM::matthews_correlation_coefficient(const Data::Contrast &contrast,
                                             bitmask_t present) const {
  // TODO is there a proper generalization of the MCC to multiple experiments?
  vector_t posterior = posterior_atleast_one(contrast, present);
  confusion_matrix m = reduce(posterior, present, contrast, false)
                       + pseudo_count;
...
vector_t HMM::expected_posterior(const Data::Contrast &contrast,
                                 bitmask_t present) const {
...
double HMM::expected_posterior(const Data::Set &dataset,
                               bitmask_t present) const {
  double m = 0;
...
#pragma omp parallel for schedule(static) reduction(+ : m) if (DO_PARALLEL)
  for (size_t i = 0; i < dataset.set_size; i++) {
    ...
    for (auto group_idx : present_groups)
      // Assume the first state of each motif is constitutive for the motif
      m += expected_state_posterior(groups[group_idx].states[0], f, b, scale);
  }
  return m;
};
...
vector_t HMM::posterior_atleast_one(const Data::Contrast &contrast,
                                    bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
...
HMM::pair_posteriors_t HMM::pair_posterior_atleast_one(
    const Data::Contrast &contrast, bitmask_t present,
    bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
...
vector_t HMM::posterior_atleast_one(const Data::Set &dataset,
                                    bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
    cout << "HMM::posterior_atleast_one(Data::Set = " << dataset.path << ")"
         << endl << "present = " << present << endl;
...
#pragma omp parallel for schedule(static) if (DO_PARALLEL)
  for (size_t i = 0; i < dataset.set_size; i++) {
    double logp = log_likelihood_from_scale(
        compute_forward_scale(dataset.sequences[i]));
    double logp_wo_motif = log_likelihood_from_scale(
        subhmm.compute_forward_scale(dataset.sequences[i]));
    double z = 1 - exp(logp_wo_motif - logp);
    if (verbosity >= Verbosity::debug)
      cout << "seq = "
           << dataset.sequences[i].definition
           /*  << " " << dataset.sequences[i].sequence */
           << " logp = " << logp << " logp_wo_motif = " << logp_wo_motif
           << " z = " << z << endl;
    vec[i] = z;
  }
  ...
  if (verbosity >= Verbosity::debug)
    cout << "HMM::posterior_atleast_one(Data::Set = " << dataset.path << ")"
         << endl << "present = " << present << endl << "vec = " << vec << endl;
  return vec;
};

enum class PairPosteriorMode { MutualPresence, Independence };

HMM::pair_posteriors_t HMM::pair_posterior_atleast_one(
    const Data::Set &dataset, bitmask_t present, bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
    cout << "HMM::pair_posterior_atleast_one(Data::Set = " << dataset.path
         << ")" << endl << "present  = " << present << endl
...
#pragma omp parallel for schedule(static) if (DO_PARALLEL)
  for (size_t i = 0; i < dataset.set_size; i++) {
    ...
    if (mode == PairPosteriorMode::MutualPresence) {
      double logp = log_likelihood_from_scale(
          compute_forward_scale(dataset.sequences[i]));
      double logp_wo_one = log_likelihood_from_scale(
          subhmm_one.compute_forward_scale(dataset.sequences[i]));
      double logp_wo_two = log_likelihood_from_scale(
          subhmm_two.compute_forward_scale(dataset.sequences[i]));
      double logp_wo_either = log_likelihood_from_scale(
          subhmm_both.compute_forward_scale(dataset.sequences[i]));
...
double HMM::sum_posterior_atleast_one(const Data::Set &dataset,
                                      bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
    cout << "HMM::sum_posterior_atleast_one(Data::Set = " << dataset.path << ")"
         << endl << "present = " << present << endl;
  ...
  if (verbosity >= Verbosity::debug)
    cout << "HMM::sum_posterior_atleast_one(Data::Set = " << dataset.path << ")"
         << endl << "present = " << present << endl << "m = " << m << endl;
  return m;
};

HMM::pair_posterior_t HMM::sum_pair_posterior_atleast_one(
    const Data::Set &dataset, bitmask_t present, bitmask_t previous) const {
  if (verbosity >= Verbosity::debug)
    cout << "HMM::sum_pair_posterior_atleast_one(Data::Set = " << dataset.path
         << ")" << endl << "present  = " << present << endl
         << "previous = " << previous << endl;

  pair_posteriors_t pair_counts
      = pair_posterior_atleast_one(dataset, present, previous);
  ...
  if (verbosity >= Verbosity::verbose
      or (verbose_conditional_mico_output and verbosity >= Verbosity::info))
    cout << "HMM::sum_pair_posterior_atleast_one(Data::Set = " << dataset.path
         << ")" << endl << "present  = " << present << endl
         << "previous = " << previous << endl
...
HMM::posterior_t HMM::posterior_atleast_one(const Data::Seq &seq,
                                            bitmask_t present) const {
  if (verbosity >= Verbosity::debug)
  ...
  double z;
  SubHMM subhmm(*this, complementary_states_mask(present));
  double logp_wo_motif
      = log_likelihood_from_scale(subhmm.compute_forward_scale(seq));

  z = 1 - exp(logp_wo_motif - logp);
  if (verbosity >= Verbosity::debug)
    cout << "seq = " << seq.definition << " " << seq.sequence
         << " logp = " << logp << " logp_wo_motif = " << logp_wo_motif
         << " z = " << z << endl;

  if (verbosity >= Verbosity::debug)
    cout << "HMM::posterior_atleast_one(Data::Seq)" << endl
         << "present = " << present << endl << "z = " << z << endl;
  posterior_t res = {logp, z};
  return res;
};

double HMM::compute_score_all_motifs(
  ...
  return compute_score(collection, measure, options, all_motifs,
                       vector<size_t>());
}

double HMM::compute_score(const Data::Collection &collection,
                          const Measures::Continuous::Measure &measure,
                          const Options::HMM &options,
                          const vector<size_t> &present_motifs,
                          const vector<size_t> &previous_motifs) const {
  double score = 0;
  double W = 0;
  double w;
  ...
  switch (measure) {
    ...
    case Measure::MutualInformation:
      for (auto &contrast : collection) {
        ...
        score += mutual_information(contrast, present)
                 * (options.weighting ? w : 1);
      }
    ...
    case Measure::ThresholdedConditionalMutualInformation: {
      ...
      for (auto &contrast : collection) {
        W += w = contrast.set_size;
        auto current = conditional_and_motif_pair_mutual_information(
            contrast, present, previous);
      ...
      if (verbosity >= Verbosity::verbose
          or (verbose_conditional_mico_output and verbosity >= Verbosity::info))
        cout << "condMI = " << conditional_mi << " pairMI = " << pair_mi
             << " ratio = " << ratio << endl;
    ...
    case Measure::ConditionalMutualInformation:
      for (auto &contrast : collection) {
        W += w = contrast.set_size;
        score += conditional_mutual_information(contrast, present, previous)
                 * (options.weighting ? w : 1);
      }
      if (options.weighting)
        score /= W;
    ...
    case Measure::PairMutualInformation:
      for (auto &contrast : collection) {
        W += w = contrast.set_size;
        score += motif_pair_mutual_information(contrast, present, previous)
                 * (options.weighting ? w : 1);
      }
      if (options.weighting)
        score /= W;
    ...
    case Measure::ClassificationLikelihood:
      ...
      break;
    default:
      cout << "Score calculation for '" << measure2string(measure)
...
double HMM::class_likelihood(const Data::Contrast &contrast, bitmask_t present,
...
double HMM::class_likelihood(const Data::Set &dataset, bitmask_t present,
                             bool compute_posterior) const {
...
bool HMM::is_present(const Data::Set &dataset, bitmask_t present) const {
  for (size_t group_idx = 0; group_idx < groups.size(); group_idx++)
    if ((bitmask_t(1 << group_idx) & present) != 0
        and dataset.motifs.find(groups[group_idx].name) != end(dataset.motifs))
      return true;
  return false;
}

double HMM::log_likelihood_difference(const Data::Contrast &contrast,
                                      bitmask_t present) const {
  double d = 0;
...
double HMM::dips_sitescore(const Data::Contrast &contrast,
                           bitmask_t present) const {
  vector_t posterior = posterior_atleast_one(contrast, present);
  confusion_matrix m = reduce(posterior, present, contrast, false)
                       + pseudo_count;
...
double HMM::dips_tscore(const Data::Contrast &contrast,
                        bitmask_t present) const {
  vector_t posterior = expected_posterior(contrast, present);
  confusion_matrix m = reduce(posterior, present, contrast, true)
                       + pseudo_count;
  size_t signal_size = m.true_positives + m.false_negatives;
  size_t control_size = m.false_positives + m.true_negatives;
