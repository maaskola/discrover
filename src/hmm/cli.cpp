...
void gen_discrover_cli(size_t cols, string &config_path, Options::HMM &options,
  ...
  basic_options_optional.add_options()
    ("score", po::value(&options.objectives)->default_value(Training::Objectives(1,default_objective), "mi"),
     ...
     "dlogl   \tLog-likelihood difference, like in the motif discovery method DME, see doi: 10.1073/pnas.0406123102; directional measure (see note below)\n"
     ...
     "\n"
     "Note that the directional measures (mcc, dlogl, and dfreq) require you to specify in which of the samples the motif is expected. "
