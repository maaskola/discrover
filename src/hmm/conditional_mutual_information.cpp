...
double calc_conditional_mutual_information(
  ...
  if (mode == conditionalMI::motif_and_condition_given_previous_motifs)
    if (verbosity >= Verbosity::verbose
      ...
      cout << "Conditional mutual information of X=new motif, and Y=contrast, "
              "given Z=previous motifs" << endl;
  if (mode == conditionalMI::motif_and_previous_motifs_given_condition) {
    if (verbosity >= Verbosity::verbose
        or (verbose_conditional_mico_output and verbosity >= Verbosity::info))
      cout << "Conditional mutual information of X=new motif, and Y=previous "
              "motifs, given Z=contrast" << endl;
    Y = 2;
  ...
  if (mode == conditionalMI::motif_and_condition_given_previous_motifs)
    ...
    for (size_t i = 0; i < contrast.sets.size(); i++) {
      ...
      p[0][i][1] = pair_posteriors[i].posterior_first
                   - pair_posteriors[i].posterior_both;
      p[1][i][0] = pair_posteriors[i].posterior_second
                   - pair_posteriors[i].posterior_both;
    ...
    for (size_t i = 0; i < contrast.sets.size(); i++) {
      ...
      p[0][1][i] = pair_posteriors[i].posterior_first
                   - pair_posteriors[i].posterior_both;
      p[1][0][i] = pair_posteriors[i].posterior_second
                   - pair_posteriors[i].posterior_both;
...
double pair_mutual_information(const Data::Contrast &contrast,
  ...
  for (size_t i = 0; i < contrast.sets.size(); i++) {
    ...
    p[0][1] += pair_posteriors[i].posterior_first
               - pair_posteriors[i].posterior_both;
    p[1][0] += pair_posteriors[i].posterior_second
               - pair_posteriors[i].posterior_both;
