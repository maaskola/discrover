...
SubHMM::SubHMM(const HMM &hmm, const Training::Range &states)
    : HMM(hmm, false),
      n_original_states(n_states),
      reduce(n_states, -1),
      lift(states) {
...
Training::Targets SubHMM::map_down(const Training::Targets &targets) const {
  Training::Targets dest
      = {map_down(targets.transition), map_down(targets.emission)};
  return dest;
}
