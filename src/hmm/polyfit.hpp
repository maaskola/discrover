 ...
 *
 *    Description:  Handle polynomial fits using least squares and find extrema
 *                  of quadratic and cubic polynomials.
 *
...
double quadratic_extremum(const vector_t &x, const vector_t &y,
                          Verbosity verbosity);
std::pair<double, double> cubic_extrema(const vector_t &x, const vector_t &y,
                                        Verbosity verbosity);
 ...
 * Return: a pair of values with the first one being the minimum, and the second
 * being the maximum.
 */
 ...
 * Return: a pair of value for the two roots, with the smaller one in the first
 * element.
...
namespace Exception {
namespace PolyFit {
struct InsufficientData : public std::runtime_error {
