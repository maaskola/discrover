...
void Registration::add_dataset(const Data::Set &dataset, double class_prior)
{
  if(datasets.find(dataset.sha1) == end(datasets)) {
    ...
    if(verbosity >= Verbosity::verbose)
      cout << "Not registering already present data_set(dataset.path=" << dataset.path << ( dataset.is_shuffle ? " shuffle" : " ") << ", sha1=" << dataset.sha1 << ", class_prior=" << class_prior << ")" << endl;
}

void Registration::add_bitmask(const std::string name, bitmask_t present, double motif_p1, double motif_p2)
{
  for(auto &sample: datasets) {
    if(sample.second.motif_prior.find(present) == end(sample.second.motif_prior)) {
      if(sample.second.spec.motifs.find(name) != end(sample.second.spec.motifs))
        sample.second.motif_prior[present] = motif_p1;
      else
        sample.second.motif_prior[present] = motif_p2;
      if(verbosity >= Verbosity::verbose)
        cout << "Adding motif prior for motif " << name << " / " << present << " in data set " << sample.second.spec.path << " = " << sample.second.motif_prior[present] << endl;
    } else
      if(verbosity >= Verbosity::verbose)
        cout << "Not adding motif prior for motif " << name << " / " << present << " in data set " << sample.second.spec.path << " = " << sample.second.motif_prior[present] << endl;
  }
}
...
double Registration::Sample::get_motif_prior(bitmask_t present) const {
  ...
  if(x == end(motif_prior)) {
    cout << "Error: could not find motif prior for motif group:" << present << "." << endl;
    throw;
  } else
    return(x->second);
}
...
double Registration::compute_marginal_motif_prior(bitmask_t present) const
{
  ...
  for(auto &x: datasets)
    marginal_motif_prior += x.second.class_prior * x.second.get_motif_prior(present);
  return(marginal_motif_prior);
}

double Registration::get_class_motif_prior(const string &sha1, bitmask_t present) const
{
  if(verbosity >= Verbosity::debug)
    cout << "get_class_motif_prior(sha1=" << sha1 << ", present=" << present << ")" << endl;
  ...
  if(x == end(cparms->second.motif_prior)) {
    cout << "Error: failed trying to access class conditional motif prior parameters of path with sha1: " << sha1 << " for the motif with index " << present << "." << endl;
    throw("Could not find conditional motif prior for registered data set.");
  }
  if(verbosity >= Verbosity::debug)
    cout << "get_class_motif_prior(sha1=" << sha1 << ", present=" << present << ") = " << x->second << endl;
  return(x->second);
}
