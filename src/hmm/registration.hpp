...
struct Registration {
  struct Sample {
    ...
    double class_prior;
    std::unordered_map<bitmask_t, double> motif_prior; // the key is supposed to be the group index of the motif
  ...
  std::unordered_map<std::string, Sample> datasets; // the keys are SHA1 hashes of the file contents

  double compute_marginal_motif_prior(bitmask_t present) const;
  double get_class_motif_prior(const std::string &sha1, bitmask_t present) const;
  double get_class_prior(const std::string &sha1) const;

  void add_dataset(const Data::Set &dataset, double class_prior);
  void add_bitmask(const std::string name, bitmask_t present, double motif_p1, double motif_p2);
};
