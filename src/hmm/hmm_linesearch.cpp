...
template <class X> X max3(const X &a, const X& b, const X&c) {
...
bool mcstep(double &stx, double &fx, double &dx, double &sty, double &fy, double &dy, double &stp, double fp, double dp, bool &brackt, double stpmin, double stpmax, Verbosity verbosity);

HMM HMM::build_trial_model(const Gradient &gradient, double alpha, const Training::Task &task) const
{
  ...
  // note: in computing the mean (that will be subtracted for centering) skip those that are negative infinite
...
inline double psi(double v1, double v2, const Gradient &g, double alpha, double mu, const Gradient &direction, Verbosity verbosity)
{
  double d = dderiv(direction, g);
  double psi = v2 - v1 - alpha * mu * d;
  if(verbosity >= Verbosity::debug)
    cout << "alpha = " << alpha << " v1 = " << v1 << " v2 = " << v2 << " mu = " << mu << " d = " << d << " psi = " << psi << endl;
  return(psi);
}

inline double psi_gradient(const Gradient &g1, const Gradient &g2, double alpha, double mu, const Gradient &direction, Verbosity verbosity)
{
  double d1 = dderiv(direction, g1);
  double d2 = dderiv(direction, g2);
  double psi = d2 - mu * d1;
  if(verbosity >= Verbosity::debug)
    cout << "alpha = " << alpha << " mu = " << mu << " d1 = " << d1 << " d2 = " << d2 << " psi gradient = " << psi << endl;
  return(psi);
}

bool sufficient_increase_condition(double v1, double v2, const Gradient &g, double alpha, double mu, const Gradient &direction, Verbosity verbosity)
{
  bool criterion = psi(v1, v2, g, alpha, mu, direction, verbosity) >= 0;
  if(verbosity >= Verbosity::debug)
    cout << "Sufficient increase criterion is " << (criterion ? "FULFILLED" : "NOT FULFILLED") << endl;
  return(criterion);
}

bool curvature_condition(double v1, double v2, const Gradient &g1, const Gradient &g2, double alpha, double eta, const Gradient &direction, Verbosity verbosity)
{
  double d1 = dderiv(direction, g1);
  double d2 = dderiv(direction, g2);
  bool criterion = fabs(d2) <= eta * fabs(d1);
  if(verbosity >= Verbosity::debug)
    cout << "Curvature criterion is " << (criterion ? "FULFILLED" : "NOT FULFILLED") << ": "
      << "|" << d2 << "| " << (criterion ? "<=" : ">") << " " << eta << " * |" << d1 << "| = " << (eta * fabs(d1)) << endl;
  return(criterion);
}
    ...
    const hmm_options &options) const
{
  ...
  if(options.line_search.max_steps == 0) {
    cout << "Error: line search option max_steps must be a positive integer." << endl;
    failed = true;
  }
  ...
  if(failed) return(pair<double,HMM>(initial_score, *this));

  // TODO determine alpha_min and alpha_max
  const double min_score = initial_score / (1 - options.termination.delta_tolerance);
  const double stpmin = (min_score - initial_score) / dderiv(direction, initial_gradient);
  ...
  if(task.measure == Measure::MutualInformation or task.measure == Measure::MatthewsCorrelationCoefficient)
    upper_limit = (1 - initial_score) / options.line_search.mu / dderiv(direction, initial_gradient); // Assume the maximal score is 1
  if(verbo >= Verbosity::debug)
  ...
  while(true) {
    ...
    if((brackt and (stp <= stmin or stp >= stmax))
        or nfev >= maxfev-1 or (brackt and stmax-stmin <= xtol * stmax))
      stp = stx;
    ...
    if((brackt and (stp <= stmin or stp >= stmax))) info = 6;
    ...
    if(f >= ftest1 and fabs(dg) <= gtol * fabs(dginit)) info = 1;

    // CHECK FOR TERMINATION.
    if(info != 0) {
      if(verbo >= Verbosity::info)
        cout << "Fnc & grad evaluations in line search          " << nfev << endl;
    ...
    if(stage1 and f >= fx and f < ftest1) {
      ...
      if(not mcstep(stx, fxm, dgxm, sty, fym, dgym, stp, fm, dgm, brackt, stmin, stmax, verbo))
      ...
      if(not mcstep(stx, fx, dgx, sty, fy, dgy, stp, f, dg, brackt, stmin, stmax, verbo))
        // the intial parameters of mcstep were wrong
        return(pair<double,HMM>(initial_score, *this));
    }
...
bool mcstep(double &stx, double &fx, double &dx, double &sty, double &fy, double &dy, double &stp, double fp, double dp, bool &brackt, double stpmin, double stpmax, Verbosity verbosity) {
  ...
  if(fp < fx) {
    ...
    bound = true;
    stpc = cubic_extrema(interpolate3(stx, stp, fx, fp, dx, dp), Verbosity::verbose).second;
    ...
    if(verbosity >= Verbosity::verbose)
      cout << "SECOND CASE. A HIGHER FUNCTION VALUE AND DERIVATIVES OF OPPOSITE SIGN." << endl;
    bound = false;
    stpc = cubic_extrema(interpolate3(stx, stp, fx, fp, dx, dp), Verbosity::verbose).second;
    ...
    if(verbosity >= Verbosity::verbose)
      cout << "THIRD CASE. A HIGHER FUNCTION VALUE, DERIVATIVES OF THE SAME SIGN, AND THE MAGNITUDE OF THE DERIVATIVE DECREASES." << endl;
    bound = true;
    ...
    // cout << "theta = " << theta << " s = " << s << " gamma = " << gamma << endl;
    // if(stp > stx) gamma = -gamma;
    // p = (gamma - dp) + theta;
    // q = (gamma + (dx - dp)) + gamma;
    // r = p/q;
    // cout << "gamma = " << gamma << " p = " << p << " q = " << q << " r = " << r << endl;
    double a_c = cubic_extrema(interpolate3(stx, stp, fx, fp, dx, dp), Verbosity::verbose).second;
...
//      stpc = stp + r*(stx - stp);
    ...
    if(verbosity >= Verbosity::verbose)
      cout << "FOURTH CASE. A HIGHER FUNCTION VALUE, DERIVATIVES OF THE SAME SIGN, AND THE MAGNITUDE OF THE DERIVATIVE DOES NOT DECREASE." << endl;
    bound = false;
    if(brackt) {
  ...
  if(verbosity >= Verbosity::verbose)
    cout << "Updated interval of uncertainty." << endl
      << "stx = " << stx << " fx = " << fx << " dx = " << dx << endl
      << "sty = " << sty << " fy = " << fy << " dy = " << dy << endl;
