...
double calc_mutual_information(const matrix_t &matrix, double pseudo_count,
  ...
  if (association_verbosity >= Verbosity::debug) {
    ...
    cout << "Calculating mutual information of this table:" << endl;
    cout << matrix << endl;
  }
  matrix_t m = matrix;
  if (pseudo_count != 0) {
...
double corrected_pvalue(double score, double n, double df, double motif_len,
  ...
  if (verbosity >= Verbosity::verbose)
    cout << "corrected_pvalue(mi=" << score << ", n=" << n << ", df=" << df
         << ", len=" << motif_len << ")" << endl;
...
double calc_log_likelihood_ratio(const matrix_t &matrix, double pseudo_count) {
  if (association_verbosity >= Verbosity::debug) {
    ...
    cout << "Calculating log likelihood ratio of this table:" << endl << matrix
         << endl;
  }
  size_t n = 0;
  for (size_t i = 0; i < matrix.size1(); i++)
...
double calc_matthews_correlation_coefficient(const confusion_matrix &m) {
  return calc_matthews_correlation_coefficient(
      m.true_positives, m.false_negatives, m.false_positives, m.true_negatives);
}
...
double calc_mutual_information(const confusion_matrix &m, bool normalize) {
  ...
  double mi
      = calc_mutual_information(m.true_positives, m.false_negatives,
                                m.false_positives, m.true_negatives, normalize);
  if (association_verbosity >= Verbosity::debug)
    cout << "mi = " << mi << endl;
  return mi;
}
...
double calc_rank_information(vector_t posterior, double pseudo_count) {
  ...
  double ric = 0;
  for (size_t i = 0; i < n - 1; i++) {
    double mi = calc_mutual_information(
        p[i] + pseudo_count, q[i] + pseudo_count, r[i] + pseudo_count,
        s[i] + pseudo_count, true);
    ric += mi;
    // cout << "debug " << i << " " << p[i] << " " << q[i] << " " << r[i] << " "
    // << s[i] << " " << p[i] + q[i] + r[i] + s[i] << " " << mi << " " << ric <<
    // endl;
  }
