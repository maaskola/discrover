...
void draw_letter_a_or_t(cairo_t *cr, const coords_t &coords, const coord_t &origin, double width, double height, rgb_t color) {
...
void draw_letter(cairo_t *cr, size_t letter, const coord_t &coord, double width, double height, const Options &options) {
  if(height > 0)
    switch(letter) {
      case 0:
...
void draw_logo_to_surface(cairo_surface_t *surface, const matrix_t &matrix, const Options &options) {
  ...
  for(auto &col: matrix) {
    ...
    if(options.order == Order::Frequency)
      sort(begin(order), end(order), [&] (size_t a, size_t b) {
          return col[a] > col[b];
          });
