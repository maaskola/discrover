...
double bd0(double x, double np)
{
  ...
  if(!std::isfinite(x) || !std::isfinite(np) || np == 0.0) return(NAN);
...
double dpois_raw(double x, double lambda, int give_log)
{
  /*       x >= 0 ; integer for dpois(), but not e.g. for pgamma()!
           lambda >= 0
           */
  if (lambda == 0) return( (x == 0) ? 
      (give_log ? 0 : 1) : (give_log ? -std::numeric_limits<double>::infinity() : 0));
  if (!std::isfinite(lambda)) return (give_log ? -std::numeric_limits<double>::infinity() : 0);
  if (x < 0) return( (give_log ? -std::numeric_limits<double>::infinity() : 0) );
  if (x <= lambda * DBL_MIN) return(give_log ?
      -lambda :
      exp(-lambda));
  if (lambda < x * DBL_MIN) return(give_log ?
      -lambda + x*log(lambda) -boost::math::lgamma(x+1) :
      //-lambda + x*log(lambda) -lgammafn(x+1) :
      exp(-lambda + x*log(lambda) -boost::math::lgamma(x+1)));
  // exp(-lambda + x*log(lambda) -lgammafn(x+1)));
  return( give_log ?
      -0.5 * log(2*M_PI*x) - stirlerr(x)-bd0(x,lambda) :
...
double dgamma(double x, double shape, double scale, int give_log)
{
  ...
  if (shape == 0) /* point mass at 0 */
    return (x == 0)? std::numeric_limits<double>::infinity() : (give_log ? -std::numeric_limits<double>::infinity() : 0);
  if (x == 0) {
    if (shape < 1) return std::numeric_limits<double>::infinity();
    if (shape > 1) return (give_log ? -std::numeric_limits<double>::infinity() : 0);
    /* else */
