  ...
  for(size_t n = 0; n <= max_motif_len; n++) {
    for(size_t k = 0; k < n_generalizations; k++) {
      if(k == 0)
        ...
        switch(k) {
          default: // A
            z(n,k) += 1 * z(n-1,k-3); // add 3 degrees of degeneracy: one IUPAC symbols allowing four nucleotides: N
          case 2:  // B
            z(n,k) += 4 * z(n-1,k-2); // add 2 degrees of degeneracy: four IUPAC symbols allowing three nucleotides: B, D, H, V
          case 1:  // C
            z(n,k) += 6 * z(n-1,k-1); // add 1 degree of degeneracy: six IUPAC symbols allowing two nucleotides: W, S, M, K, R, Y
          case 0:  // D
            z(n,k) += 4 * z(n-1,k);   // add 0 degrees of degeneracy: four IUPAC symbols allowing one nucleotides: A, C, G, T
        }
      }
    }
  }
