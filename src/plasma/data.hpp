...
struct Set : public Specification::Set {
  ...
  Set() : Specification::Set(), seq_size(0), set_size(0), sequences(){};
  Set(const Specification::Set &s, bool revcomp = false, size_t n_seq = 0)
      : Specification::Set(s), seq_size(0), set_size(0), sequences() {
  ...
  RemovalReport drop_sequences(const mask_sub_t &mask) {
    ...
    while (iter != sequences.rend()) {
      if (noisy_output) {
        ...
        std::cerr << "Checking " << path << " " << iter->definition
                  << " for dropping" << std::endl;
      }
      if (mask.find(iter->definition) != end(mask)) {
        ...
        set_size--;
        seq_size -= iter->sequence.size();  // TODO: find out if this is correct
                                            // for reverse complements
        auto i = iter;
...
struct Contrast {
  ...
  Contrast(const std::string &name_, const Specification::Sets &specs,
           bool revcomp = false, size_t n_seq = 0)
      : seq_size(0), set_size(0), sets(), name(name_) {
    for (auto &spec : specs)
  ...
  Contrast(const Contrast<Y> &contrast)
      : seq_size(0), set_size(contrast.set_size), sets(), name(contrast.name) {
  ...
  std::vector<std::string> save_shuffle_sequences(const std::string &stem,
                                                  size_t &idx) const {
...
struct Collection {
  ...
  Collection() : seq_size(0), set_size(0), contrasts(){};
  Collection(const Specification::Sets &specs, bool revcomp = false,
             size_t n_seq = 0)
      : seq_size(0), set_size(0), contrasts() {
  ...
  Collection(const Collection<Y> &coll)
      : seq_size(0), set_size(coll.set_size), contrasts() {
  ...
  iterator find(const std::string &s) {
    iterator iter = std::find_if(
        begin(contrasts), end(contrasts),
        [&s](const contrast_t &contrast) { return contrast.name == s; });
    return iter;
  }
  ...
  std::vector<std::string> save_shuffle_sequences(
...
template <typename X>
typename Set<X>::iterator begin(Set<X> &set) {
  return begin(set.sequences);
}
template <typename X>
typename Set<X>::iterator end(Set<X> &set) {
  return end(set.sequences);
}
template <typename X>
typename Set<X>::const_iterator begin(const Set<X> &set) {
  return begin(set.sequences);
}
template <typename X>
typename Set<X>::const_iterator end(const Set<X> &set) {
  return end(set.sequences);
}

template <typename X>
typename Contrast<X>::iterator begin(Contrast<X> &contrast) {
  return begin(contrast.sets);
}
template <typename X>
typename Contrast<X>::iterator end(Contrast<X> &contrast) {
  return end(contrast.sets);
}
template <typename X>
typename Contrast<X>::const_iterator begin(const Contrast<X> &contrast) {
  return begin(contrast.sets);
}
template <typename X>
typename Contrast<X>::const_iterator end(const Contrast<X> &contrast) {
  return end(contrast.sets);
}

template <typename X>
typename Collection<X>::iterator begin(Collection<X> &collection) {
  return begin(collection.contrasts);
}
template <typename X>
typename Collection<X>::iterator end(Collection<X> &collection) {
  return end(collection.contrasts);
}
template <typename X>
typename Collection<X>::const_iterator begin(const Collection<X> &collection) {
  return begin(collection.contrasts);
}
template <typename X>
typename Collection<X>::const_iterator end(const Collection<X> &collection) {
  return end(collection.contrasts);
}
}
}
...
struct Set : public Data::Basic::Set<Fasta::Entry> {
  ...
  Set(const Specification::Set &s, bool revcomp = false, size_t n_seq = 0);
  template <typename X>
  Set(const Data::Basic::Set<X> &s)
      : Data::Basic::Set<Fasta::Entry>(s){};
};

struct Contrast : public Data::Basic::Contrast<Set> {
  Contrast();
  Contrast(const std::string &name, const Specification::Sets &s,
           bool revcomp = false, size_t n_seq = 0);
  template <typename X>
  Contrast(const Data::Basic::Contrast<X> &s)
      : Data::Basic::Contrast<Set>(s){};
};

struct Collection : public Data::Basic::Collection<Contrast> {
  Collection(const Specification::Sets &s, bool revcomp = false,
             size_t n_seq = 0);
  template <typename X>
  Collection(const Data::Basic::Collection<X> &c)
      : Data::Basic::Collection<Contrast>(c){};
};
};
