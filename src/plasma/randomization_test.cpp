...
count_vector_t generate_sample(const Seeding::Collection &collection,
  ...
  for (auto &contrast : collection) {
    ...
    if (verbosity >= Verbosity::debug)
      cout << "Considering contrast with count start index " << start_idx
           << " and stop index " << stop_idx << endl;
    ...
    if (verbosity >= Verbosity::debug)
      cout << "In total there are " << successes << " in this contrast of "
           << remaining << " sequences." << endl;
...
bool randomization_test(const Seeding::Collection &collection,
                        const count_vector_t &counts, size_t nr_tests,
                        double score, const Seeding::Options &options,
                        const Seeding::Objective &objective, size_t length,
                        size_t degeneracy) {
  if (options.verbosity >= Verbosity::verbose)
    cout << "Randomization test" << endl;
  for (size_t i = 0; i < nr_tests; i++) {
    count_vector_t sample
        = generate_sample(collection, counts, options.verbosity);

    double sample_score = compute_score(collection, sample, options, objective,
                                        length, degeneracy);
