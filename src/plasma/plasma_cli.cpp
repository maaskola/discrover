...
boost::program_options::options_description gen_iupac_options_description(Seeding::Options &options,
  ...
  if(include_all)
    desc.add_options()
      ...
      ("pdf", po::bool_switch(&options.pdf_logo), "Generate PDF files for the found motifs.")
      ("png", po::bool_switch(&options.png_logo), "Generate PNG files for the found motifs.")
