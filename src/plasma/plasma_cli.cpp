...
boost::program_options::options_description gen_iupac_options_description(Seeding::Options &options,
  ...
  desc.add_options()
    (form_switch(prefix, "algo", allow_short).c_str(), po::value<Seeding::Algorithm>(&options.algorithm)->default_value(Seeding::Algorithm::Plasma, "plasma"), "Which algorithm to use for seeding. Available are 'plasma', 'dreme', 'fire', 'mcmc', and 'all'. Multiple algorithms can be used by separating them by comma.")
