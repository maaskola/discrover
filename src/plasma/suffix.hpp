...
std::vector<idx_t> gen_suffix_array_slow(Iter begin, Iter end, Verbosity verbosity) {
  Timer timer;
...
std::vector<lcp_t> gen_lcp_slow(Iter begin, Iter end, const std::vector<idx_t> &sa, Verbosity verbosity) {
  Timer timer;
  idx_t n = sa.size();
 ...
 */
template <class lcp_t, class idx_t, class Iter>
std::vector<lcp_t> gen_lcp(Iter begin, Iter end, const std::vector<idx_t> &sa, Verbosity verbosity) {
  Timer timer;
...
std::vector<idx_t> match(Iter qbegin, Iter qend, Iter begin, Iter end, const std::vector<idx_t> &sa, const std::vector<lcp_t> &lcp, const std::vector<idx_t> &jmp, Cmp cmp=Cmp()) {
  ...
  idx_t i = 0;
  while(i < n) {
    if(do_debug) std::cout << "i = " << i << std::endl;
    ...
    auto q = qbegin + j;
    auto t = begin + sa[i] + j;
    if(do_debug) std::cout << "cmp = " << *t << " " << *q << std::endl;
    while(q != qend and t != end and cmp(*t, *q)) {
      if(do_debug) std::cout << "matched = " << *t << " " << *q << std::endl;
      q++; t++;
      if(do_debug and q != qend and t != end) std::cout << "cmp = " << *t << " " << *q << std::endl;
    }
    if(q == qend) { // found a match
      if(do_debug) std::cout << "adding hit: " << sa[i] << std::endl;
      hits.push_back(sa[i]);
      i++;
      while(i < n and lcp[i] >= qsize) {
        ...
        for( ; j < i; j++) {
          hits.push_back(sa[j]);
          if(do_debug) std::cout << "adding subsequent hit: " << sa[j] << std::endl;
        }
      }
    } else if(q == qbegin + j) { // mismatch on first comparison
      i = jmp_stack.top();
      jmp_stack.pop();
      if(do_debug) std::cout << "Jump!" << std::endl;
    } else { // mismatch on later position
      i++;
      size_t k = std::distance(qbegin, q);
      if(do_debug) std::cout << "i = " << i << " k = " << k << " lcp = " << lcp[i] << std::endl;
      while(i < n and lcp[i] > k) {
        i = jmp[i];
        if(do_debug) std::cout << "Bounce!" << std::endl;
        if(do_debug) std::cout << "i = " << i << " k = " << k << " lcp = " << lcp[i] << std::endl;
      }
    }
  }
