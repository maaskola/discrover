...
boost::program_options::options_description gen_plasma_options_description(
  ...
  desc.add_options()
    (form_switch(prefix, "algo", false).c_str(), po::value<Seeding::Algorithm>(&options.algorithm)->default_value(Seeding::Algorithm::Plasma, "plasma"), (string() + "Seeding algorithm. Available are 'plasma', 'mcmc', " + (DREME_FOUND ? "'dreme', " : "") + "and 'all'. Multiple algorithms can be used by separating them by comma.").c_str())
    ...
    (form_switch(prefix, "cand", false).c_str(), po::value<size_t>(&options.plasma.max_candidates)->default_value(100), "How many candidates to maintain.")
  ...
  if (include_all) {
    ...
    mcmc_desc.add_options()
      ("temp", po::value<double>(&options.mcmc.temperature)->default_value(1e-3), "When performing MCMC sampling use this temperature. The temperatures of parallel chains is decreasing by factors of two.")
