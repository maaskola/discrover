...
vector<size_t> shift_positions_left(const vector<size_t> original_inserts, size_t shift)
{
...
vector<size_t> shift_positions_right(const vector<size_t> original_inserts, size_t shift, size_t maxp)
{
  vector<size_t> inserts;
  for(auto p : original_inserts)
    if(p < maxp - shift)
